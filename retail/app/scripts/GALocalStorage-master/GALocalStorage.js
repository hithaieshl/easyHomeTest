...
(function () {
	...
	ga_storage = new function () {
        ...
        // public
		this._trackTiming = function (category, variable, time, opt_label, opt_sampleRate, source, medium, campaign) {
			if (IS_DEBUG) {
				console.log('Track Timing', arguments);
			}

			request_cnt++;
			var event = '14(90!' + variable + '*' + category + '*' + time;
			if (opt_label) {
				event += '*' + opt_label + ')';
			} else {
				event += ')';
